    ...
    BOARD_DATA.forEach((space, index) => {
        ...
        if (space.type === 'sesmt') {
            content = `<div class="inner-text sesmt-space-inner">
                         ...
                         <span class="sesmt-price">$${space.price}</span>
                       </div>`;
        }
...
export function setSpaceOwnerTagDisplayHTMLBadgeVisual(indexSpaceId, colorStrIdHexPlay) {
...
export function setSipatBadgeOnSpace(spaceId) {
    ...
    badge.innerHTML = '🎉';
    badge.title     = 'SIPAT — Aluguel dobrado!';
 ...
 */
export async function animateDiceOnBoard(d1, d2, isDouble) {
